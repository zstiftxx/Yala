...
- **Cursos sin dato de prerrequisito**: un curso de ciclo 3+ sin prerrequisitos declarados no es lo mismo que "no tiene prerrequisitos" — los PDF no siempre los traen. `sinDatosDePrerequisito(carrera)` (en `mallaCurricular.js`) es la fuente única de esa distinción; la usan el Dashboard (los aparta en "Sin prerrequisitos registrados") y el mapa. En el mapa esos cursos van con **borde punteado + un "?"** en la esquina y su propia entrada en la leyenda; sin eso, un curso sin flechas se veía igual que un hueco de datos. El "?" además del borde es a propósito: el color/estilo solo no comunica a quien no lo distingue. En ciclos 1-2 la ausencia sí es real, así que quedan fuera. Se nota sobre todo en Ing. Ambiental (17 de 41) y Mecatrónica.
