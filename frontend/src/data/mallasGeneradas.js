// Generado desde los planes de estudio (PDF) de la Universidad de Lima.
// Ciclos 3+ por carrera; los ciclos 1-2 se toman de cursosGenerales.js.
// Solo cursos obligatorios (sin electivos).
//
// NO EDITAR A MANO: regenerar con `python scripts/extraer_mallas.py`.
// Los PDF de origen estan en scripts/pdf/.

export const cursosAvanzadosGenerados = {
  "Administración": {
    "3": [
      "Introducción a las Finanzas",
      "Contabilidad General",
      "Matemática para la Gestión de Negocios",
      "Estadística para la Gestión Empresarial I"
    ],
    "4": [
      "Planeamiento Estratégico I",
      "Costos y Presupuestos",
      "Estadística para la Gestión Empresarial II",
      "Herramientas Informáticas Para La Gestión II"
    ],
    "5": [
      "Diseño Organizacional",
      "Gestión de Personas",
      "Elaboración de Estados Financieros",
      "Gestión Legal Empresarial",
      "Marketing Estratégico",
      "Macroeconomía"
    ],
    "6": [
      "Planeamiento Estratégico II",
      "Análisis de la Información Financiera",
      "Comportamiento Organizacional",
      "Investigación y Análisis de Mercado",
      "Administración de Operaciones",
      "Tecnologías de Información para la Gestión"
    ],
    "7": [
      "Taller de Innovación y Creatividad Empresarial",
      "Logística y Comercio Internacional",
      "Gestión Financiera",
      "Taller de Liderazgo Y Habilidades Gerenciales",
      "Legislación Laboral",
      "Gestión Comercial"
    ],
    "8": [
      "Taller de Emprendimiento y Creación De Negocios",
      "Gestión De La Cadena De Suministro",
      "Mercado De Valores",
      "Ética Y Sostenibilidad Empresarial",
      "Taller De Investigación Empresarial"
    ],
    "9": [
      "Inteligencia De Negocios",
      "Gerencia De Operaciones",
      "Gerencia Financiera",
      "Seminario De Investigación En Administración I"
    ],
    "10": [
      "Dirección Estratégica",
      "Gerencia De Personas",
      "Seminario De Investigación En Administración II",
      "Gerencia De Marketing"
    ]
  },
  "Arquitectura": {
    "3": [
      "Dibujo III",
      "Edificación Sostenible I",
      "Física",
      "Introducción a la Arquitectura",
      "Urbanismo y Medio Ambiente I",
      "Proyecto de Arquitectura III"
    ],
    "4": [
      "Edificación Sostenible II",
      "Grafica Digital",
      "Historia y Teoría de la Arquitectura I",
      "Orientación Estructural",
      "Tecnología Ambiental I",
      "Proyecto de Arquitectura IV"
    ],
    "5": [
      "Dibujo y presentación de Proyectos",
      "Diseño Estructural I",
      "Historia y Teoría de la Arquitectura II",
      "Instalaciones I",
      "Urbanismo y Medio Ambiente II",
      "Proyecto de Arquitectura V"
    ],
    "6": [
      "Diseño Estructural II",
      "Historia y Teoría de la Arquitectura III",
      "Instalaciones II",
      "Estadística",
      "Proyecto de Arquitectura VI"
    ],
    "7": [
      "Gestión de Proyectos I",
      "Historia de la Arquitectura Peruana",
      "Metrados, Presupuesto y Programación de Obra",
      "Proyecto de Arquitectura VII"
    ],
    "8": [
      "Desarrollo de Proyecto",
      "Gestión de Proyectos II",
      "Proyecto de Arquitectura VIII"
    ],
    "9": [
      "Gestión de Proyectos IV",
      "Proyecto de Fin de Carrera",
      "Seminario de Investigación en Arquitectura"
    ]
  },
  "Comunicaciones": {
    "3": [
      "Storytelling",
      "Gamificación",
      "Fundamentos de la Comunicación",
      "Estéticas Contemporáneas",
      "Lenguaje Gráfico",
      "Lenguaje Audiovisual",
      "Fundamentos de Publicidad y Marketing"
    ],
    "4": [
      "Bases de la Información",
      "Comunicación y Desarrollo",
      "Técnicas Fotográficas",
      "Comunicación y Empresa",
      "Técnicas Audiovisuales"
    ],
    "5": [
      "Sociedad y Comunicación",
      "Narrativa Audiovisual",
      "Técnicas de Audio",
      "Técnicas de Diseño"
    ],
    "6": [
      "Técnicas de Investigación",
      "Taller de Video",
      "Comunicación y Experiencia de Usuario"
    ],
    "7": [
      "Procesos Interculturales",
      "Comunicación Política",
      "Educación y Comunicación",
      "Semiótica Narrativa"
    ],
    "8": [
      "Gestión Empresarial"
    ],
    "9": [
      "Legislación y Ética Profesional"
    ]
  },
  "Contabilidad y Finanzas": {
    "3": [
      "Fundamentos de Administración",
      "Estadística para la Gestión Empresarial I",
      "Fundamentos de Finanzas",
      "Contabilidad General",
      "Herramientas Informáticas para la Gestión I"
    ],
    "4": [
      "Estadística para la Gestión Empresarial II",
      "Planeamiento Estratégico",
      "Microeconomía",
      "Costos y Presupuestos",
      "Herramientas Informáticas para la Gestión II",
      "Principios de Contabilidad Financiera"
    ],
    "5": [
      "Macroeconomía",
      "Elaboración de Estados Financieros",
      "Contabilidad Gerencial",
      "Gestión Legal Empresarial",
      "Diseño Organizacional",
      "Gestión Tributaria"
    ],
    "6": [
      "Análisis y Planeamiento Financiero",
      "Auditoría Interna y Cumplimiento",
      "Contabilidad Financiera Intermedia",
      "Impuesto a la Renta",
      "Tecnologías de la Información para la Gestión"
    ],
    "7": [
      "Auditoría Financiera I",
      "Finanzas Corporativas I",
      "Legislación Laboral",
      "Impuestos Indirectos",
      "Taller de Liderazgo y Habilidades Gerenciales",
      "Contabilidad Internacional para el Sector Público",
      "Gestión Bancaria",
      "Seminario de Informática Contable"
    ],
    "8": [
      "Banca de Inversión",
      "Introducción a la Investigación Contable",
      "Contabilidad Financiera Avanzada",
      "Taller de Emprendimiento y Creación de Negocios",
      "Ética y Sostenibilidad Empresarial",
      "Gerencia de Activos Financieros",
      "Tributación Sectorial",
      "ERP para la Gestión Empresarial",
      "Auditoría Forense"
    ]
  },
  "Derecho": {
    "3": [
      "Expresión Escrita",
      "Problemática Nacional",
      "Historia General del Derecho",
      "Introducción a la Ciencia Política",
      "Introducción a las Ciencias Jurídicas",
      "Derecho Civil I (Principios Generales y Personas Naturales)"
    ],
    "4": [
      "Derecho Civil II (Personas Jurídicas)",
      "Introducción al Derecho Procesal",
      "Derecho Romano",
      "Derecho Penal I (Parte General)",
      "Derecho Constitucional I (Derecho Constitucional General)",
      "Lógica y Argumentación Jurídica"
    ],
    "5": [
      "Derecho Penal II",
      "Derecho Civil III (Acto Jurídico)",
      "Derecho Constitucional II (Derecho Constitucional Peruano)",
      "Filosofía del Derecho",
      "Derecho Procesal Civil I",
      "Introducción a la Economía y Derecho"
    ],
    "6": [
      "Derecho Administrativo I",
      "Derecho Procesal Penal I",
      "Derecho Procesal Constitucional",
      "Derecho Civil IV (Reales)",
      "Derecho Procesal Civil II",
      "Introducción a la Contabilidad"
    ],
    "7": [
      "Derecho Civil V (Familia)",
      "Derecho Administrativo II",
      "Derecho Comercial I (Parte General)",
      "Derecho Procesal Penal II",
      "Derecho Civil VI (Obligaciones)",
      "Derecho Procesal Civil III"
    ],
    "8": [
      "Derecho Civil VIII (Sucesiones)",
      "Derecho Laboral I (Parte General)",
      "Derecho Comercial II (Sociedades I)",
      "Derecho Civil VII (Contratos Parte General)",
      "Derecho Tributario I (Código Tributario)"
    ],
    "9": [
      "Derecho Tributario II",
      "Derecho Civil IX (Contratos Típicos)",
      "Derecho Penal III (Penal Económico)",
      "Derecho Comercial III (Sociedades II)",
      "Derecho Laboral II (Derecho Laboral Individual)"
    ],
    "10": [
      "Derecho Civil X (Garantías)",
      "Seminario de Investigación I",
      "Derecho Internacional Público",
      "Arbitraje",
      "Derecho Comercial IV (Títulos Valores)"
    ],
    "11": [
      "Ética Profesional",
      "Derecho Registral y Notarial",
      "Derecho Civil XI (Responsabilidad Civil)"
    ],
    "12": [
      "Introducción a las Técnicas de Litigación Oral",
      "Seminario de Investigación II",
      "Derecho Internacional Privado"
    ]
  },
  "Economía": {
    "3": [
      "Contabilidad para Economistas",
      "Métodos Matemáticos I",
      "Estadística General",
      "Historia del Pensamiento Económico",
      "Herramientas Informáticas para Economistas",
      "Introducción a la Microeconomía"
    ],
    "4": [
      "Teoría Microeconómica I",
      "Estadística Aplicada I",
      "Teoría Macroeconómica I",
      "Métodos Matemáticos II",
      "Ciencia de Datos",
      "Historia de la Economía Peruana"
    ],
    "5": [
      "Teoría Microeconómica II",
      "Estadística Aplicada II",
      "Teoría Macroeconómica II",
      "Métodos Matemáticos III",
      "Matemáticas Financieras y Actuariales"
    ],
    "6": [
      "Análisis Financiero",
      "Economía Internacional",
      "Teoría Macroeconómica III",
      "Métodos Matemáticos IV",
      "Teoría de Juegos",
      "Econometría I"
    ],
    "7": [
      "Economía Financiera I",
      "Organización Industrial",
      "Teoría y Política Monetaria",
      "Economía Peruana Sectorial",
      "Teoría Bancaria",
      "Econometría II"
    ],
    "8": [
      "Evaluación Económica de Políticas",
      "Economía de la Elección Pública",
      "Crecimiento y Desarrollo Económico",
      "Mercado de Capitales",
      "Tópicos Avanzados en Economía y Finanzas",
      "Economía Financiera II"
    ],
    "9": [
      "Seminario de Investigación I",
      "Política Económica",
      "Economía Financiera Aplicada",
      "Ética para Economistas",
      "Evaluación Privada de Proyectos"
    ],
    "10": [
      "Seminario de Investigación II",
      "Tópicos en Investigación Económica"
    ]
  },
  "Ingeniería Ambiental": {
    "3": [
      "Inteligencia Artificial Aplicada",
      "Sistemas de Información Geográfica",
      "Fundamentos de Programación",
      "Física I",
      "Química General",
      "Cálculo II"
    ],
    "4": [
      "Principios de Ingeniería Sostenible",
      "Estadística y Probabilidad",
      "Biología Ambiental",
      "Física II",
      "Química Ambiental",
      "Cálculo III"
    ],
    "5": [
      "Estadística Aplicada",
      "Derecho y Legislación Ambiental",
      "Cambio Climático",
      "Termodinámica",
      "Ciencia del Suelo",
      "Técnicas Instrumentales de Análisis"
    ],
    "6": [
      "Gestión Sostenible de Residuos",
      "Costeo de Proyectos Ambientales",
      "Técnicas de Control de Contaminación del Aire",
      "Energías Renovables",
      "Mecánica de Fluidos",
      "Microbiología Ambiental"
    ],
    "7": [
      "Medio Ambiente y Sociedad",
      "Recursos Naturales y Ecosistemas",
      "Técnicas de Control de la Contaminación del Agua",
      "Ética y Responsabilidad Social",
      "Sistemas Integrados de Gestión",
      "Economía Ambiental"
    ],
    "8": [
      "Evaluación del Impacto Ambiental",
      "Hidrología",
      "Técnicas de Control de la Contaminación del Suelo",
      "Inteligencia de Negocios",
      "Gestión de Recursos Energéticos"
    ],
    "9": [
      "Ecotoxicología Ambiental",
      "Proyecto de Ingeniería Aplicada I",
      "Formulación y Diseño de Proyectos Sostenibles"
    ],
    "10": [
      "Proyecto de Ingeniería Aplicada II",
      "Restauración y Compensación Ambiental",
      "Gestión de Proyectos"
    ]
  },
  "Ingeniería Civil": {
    "3": [
      "Cálculo II",
      "Topografía",
      "Probabilidad y Estadística Aplicada",
      "Ingeniería Gráfica",
      "Química Aplicada",
      "Física I",
      "Inteligencia Artificial Aplicada"
    ],
    "4": [
      "Tecnología de Construcción I",
      "Geología Aplicada",
      "Modelación de Información de Edificaciones I",
      "Física II",
      "Ecuaciones Diferenciales",
      "Tecnología del Concreto"
    ],
    "5": [
      "Programación Digital",
      "Mecánica de Suelos I",
      "Dinámica",
      "Modelación de Información de Edificaciones II",
      "Estática",
      "Tecnología de Construcción II"
    ],
    "6": [
      "Ingeniería Ambiental",
      "Mecánica de Suelos II",
      "Tecnología de los Materiales",
      "Instalaciones en Edificaciones",
      "Resistencia de Materiales",
      "Costos y Presupuestos"
    ],
    "7": [
      "Mecánica de Fluidos",
      "Gestión de Proyectos de Construcción",
      "Ingeniería de Transporte I",
      "Fundamentos del Planeamiento Urbano y Regional",
      "Análisis Estructural I",
      "Ética, Liderazgo y Responsabilidad Social",
      "Métodos Numéricos"
    ],
    "8": [
      "Hidráulica",
      "Gestión de Proyectos de Operación y Mantenimiento",
      "Gestión Estratégica de Contratos",
      "Concreto Armado I",
      "Ingeniería Sismorresistente"
    ],
    "9": [
      "Hidrología",
      "Seminario De Investigación I"
    ],
    "10": [
      "Seminario de Investigación II",
      "Estructuras Metálicas"
    ]
  },
  "Ingeniería Industrial": {
    "3": [
      "Sistemas Organizacionales",
      "Diseño Asistido por el Computador",
      "Inteligencia Artificial Aplicada",
      "Física I",
      "Química General",
      "Cálculo II"
    ],
    "4": [
      "Costeo de Operaciones",
      "Fundamentos de Programación",
      "Física II",
      "Estadística y Probabilidad",
      "Mecánica",
      "Cálculo III"
    ],
    "5": [
      "Investigación de Operaciones I",
      "Fundamentos de Operaciones y Logística",
      "Ingeniería Económica",
      "Electricidad y Electrónica",
      "Ecuaciones Diferenciales",
      "Diseño de Experimentos"
    ],
    "6": [
      "Investigación de Operaciones II",
      "Ergonomía y Diseño del Trabajo",
      "Innovación en Ingeniería",
      "Planeamiento y Control de Operaciones",
      "Procesos Industriales"
    ],
    "7": [
      "Ingeniería Financiera",
      "Diseño de Instalaciones",
      "Inteligencia de Negocios",
      "Modelos de Sistemas Logísticos",
      "Calidad",
      "Procesos de Manufactura"
    ],
    "8": [
      "Simulación de Procesos",
      "Modelamiento Predictivo de Datos",
      "Sistemas Integrados de Gestión",
      "Gestión de Proyectos",
      "Análisis de Problemas de Ingeniería",
      "Automatización Industrial"
    ],
    "9": [
      "Ética y Gestión Humana",
      "Proyecto de Ingeniería Aplicada I",
      "Ingeniería Comercial"
    ],
    "10": [
      "Proyecto de Ingeniería Aplicada II",
      "Gerencia Estratégica"
    ]
  },
  "Ingeniería Mecatrónica": {
    "3": [
      "Inteligencia Artificial Aplicada",
      "Cálculo II",
      "Circuitos Digitales",
      "Sistemas Organizacionales",
      "Dibujo Mecánico"
    ],
    "4": [
      "Ecuaciones Diferenciales",
      "Estadística y Probabilidad",
      "Circuitos Eléctricos",
      "Fundamentos de Máquinas y Mecanismos",
      "Química General"
    ],
    "5": [
      "Análisis Complejo y Transformadas",
      "Costos y Presupuestos",
      "Programación de Computadoras",
      "Mecánica Aplicada",
      "Ingeniería de Resistencia de Materiales",
      "Circuitos Electrónicos"
    ],
    "6": [
      "Análisis de Señales y Sistemas",
      "Tecnología de Materiales e Innovación",
      "Control de Sistemas Dinámicos I",
      "Ingeniería de Fluidos y Calor",
      "Desarrollo de Competencias Gerenciales",
      "Microcontroladores",
      "Sensores y Actuadores Inteligentes"
    ],
    "7": [
      "Diseño y Manufactura Asistida por Computadora",
      "Máquinas Eléctricas",
      "Aprendizaje de Máquinas",
      "Sistemas Embebidos e IOT Industrial",
      "Control de Sistemas Dinámicos II",
      "Formulación y Evaluación de Proyectos"
    ],
    "8": [
      "Diseño de Sistemas Mecatrónicos",
      "Proyecto de Investigación",
      "Control de Procesos Industriales",
      "Manufactura Integrada por Computadora",
      "Procesamiento Digital de Imágenes"
    ],
    "9": [
      "Diseño de Plantas Industriales",
      "Proyecto Integrador de Mecatrónica I",
      "Gestión de Proyectos"
    ],
    "10": [
      "Sistemas Robóticos Industriales",
      "Proyecto Integrador de Mecatrónica II"
    ]
  },
  "Marketing": {
    "3": [
      "Estadística Empresarial I",
      "Contabilidad General",
      "Antropología del Consumidor",
      "Introducción a las Finanzas",
      "Matemática para la Gestión de Negocios",
      "Herramientas Informáticas para la Gestión I"
    ],
    "4": [
      "Costos y Presupuestos",
      "Sociología del Consumidor",
      "Procesos de Marketing",
      "Estadística Empresarial II",
      "Administración Empresarial y Marketing",
      "Microeconomía"
    ],
    "5": [
      "Estadística Aplicada al Marketing",
      "Consumidor y Experiencia del Cliente",
      "Elaboración de Estados Financieros",
      "Administración de Canales de Distribución",
      "Gestión de Producto y Marca",
      "Macroeconomía"
    ],
    "6": [
      "Política y Fijación de Precios",
      "Operaciones Logísticas de Marketing",
      "Investigación Cuantitativa de Mercados",
      "Publicidad y Medios",
      "Branding",
      "Investigación Cualitativa de Mercados",
      "Ciencia de Datos Aplicado al Marketing"
    ],
    "7": [
      "Dirección Estratégica de Marketing",
      "Finanzas para Marketing",
      "Marketing Social",
      "Comunicación e Imagen Corporativa",
      "Transformación Digital y Marketing",
      "Endomarketing y Gestión de Personas"
    ],
    "8": [
      "Gerencia de Ventas",
      "Ética y Sostenibilidad Empresarial",
      "Herramientas Estratégicas de Negocios y Marketing",
      "Métodos y Técnicas de Evaluación de Proyectos",
      "E-commerce"
    ],
    "9": [
      "Trade Marketing y Merchandising",
      "Marketing de Servicios",
      "Seminario de Investigación en Marketing I",
      "Analítica de Datos",
      "Negociación Comercial"
    ],
    "10": [
      "Seminario de Investigación en Marketing II",
      "Estrategias de Marketing Internacional",
      "Estrategia de Relacionamiento y Fidelización",
      "Plan de Marketing",
      "Planeamiento Estratégico Publicitario"
    ]
  },
  "Negocios Internacionales": {
    "3": [
      "Estadística para la Gestión Empresarial I",
      "Cross-Cultural Management",
      "Introduction to International Business",
      "Matemática para la Gestión de Negocios",
      "International Relations",
      "Contabilidad General"
    ],
    "4": [
      "Regulation of International Business Transactions",
      "Microeconomía",
      "Estadística para la Gestión Empresarial II",
      "Economic Integration Processes",
      "Marketing Internacional, Estudio de Mercado y Estrategias de Ventas",
      "Costos y Presupuestos",
      "International Business Workshop I"
    ],
    "5": [
      "Exports, Imports and Customs",
      "Macroeconomía",
      "Métodos Cuantitativos para la Investigación en Negocios Internacionales",
      "Inteligencia en los Negocios y Big Data",
      "Inteligencia de Mercados Internacionales",
      "Emprendimiento de Negocios Globales y Digitales",
      "International Business Workshop II"
    ],
    "6": [
      "International Logistics I",
      "Teoría del Comercio Internacional",
      "International Business Strategies and Sustainability",
      "Desarrollo e Innovación de Servicios Internacionales",
      "Comportamiento de Mercados Internacionales",
      "Finanzas Empresariales I",
      "Data Analytics Aplicado a los Negocios Internacionales",
      "Taller de Inteligencia de Mercados",
      "Cooperación Internacional para el Desarrollo Sostenible"
    ],
    "7": [
      "International Logistics II",
      "Estrategia de Producción y Precios Internacionales",
      "Global Standards Requirements",
      "Estrategias Aplicadas para la Negociación Internacional",
      "Innovation and International Product Development",
      "Finanzas Empresariales II",
      "Taller de Liderazgo y Habilidades Gerenciales"
    ],
    "8": [
      "Dirección Estratégica Internacional",
      "Dirección Logística Global",
      "International Corporate Finance",
      "Seminario de Investigación en Negocios Internacionales II",
      "Integración Económica Comercial Asia Pacífico",
      "Gestión de Operaciones Globales II",
      "Gestión del Riesgo en Proyectos Internacionales"
    ]
  },
  "Psicología": {
    "3": [
      "REALIDAD NACIONAL",
      "REDACCIÓN CIENTÍFICA",
      "CONDUCTA SOCIAL",
      "PROCESOS COGNITIVOS BÁSICOS",
      "FILOSOFÍA DE LA CIENCIA",
      "PSICOBIOLOGÍA HUMANA I"
    ],
    "4": [
      "PROCESOS COGNITIVOS SUPERIORES",
      "DESARROLLO HUMANO I",
      "PSICOLOGÍA DE LA PERSONALIDAD",
      "PSICOBIOLOGÍA HUMANA II",
      "SEXUALIDAD HUMANA",
      "HISTORIA Y SISTEMAS PSICOLÓGICOS"
    ],
    "5": [
      "PSICOLOGÍA DEL APRENDIZAJE",
      "DESAROLLO HUMANO II",
      "ESTADÍSTICA APLICADA A LA PSICOLOGÍA",
      "TÉCNICAS DE ENTREVISTA Y OBSERVACIÓN",
      "PSICOLOGÍA DE LA SALUD",
      "TÉCNICAS DE GRUPOS"
    ],
    "6": [
      "EVALUACIÓN PSICOMÉTRICA DE PROCESOS COGNITIVOS",
      "METODOLOGÍA DE LA INVESTIGACIÓN CUANTITATIVA",
      "PSICOPATOLOGÍA",
      "COMPORTAMIENTO ORGANIZACIONAL I",
      "EPISTEMOLOGÍA DE LA PSICOLOGÍA"
    ],
    "7": [
      "PSICOLOGÍA EDUCATIVA",
      "EVALUACIÓN PSICOMÉTRICA DE LA PERSONALIDAD",
      "METODOLOGÍA DE LA INVESTIGACIÓN CUALITATIVA",
      "EVALUACIÓN PROYECTIVA DE LA PERSONALIDAD",
      "COMPORTAMIENTO ORGANIZACIONAL II"
    ],
    "8": [
      "EVALUACIÓN PSICOPEDAGÓGICA",
      "DISEÑO Y CONSTRUCCIÓN DE INSTRUMENTOS PSICOLÓGICOS",
      "INTRODUCCIÓN A LA PSICOTERAPIA I",
      "PSICOLOGÍA COMUNITARIA",
      "PROCESOS DE RECURSOS HUMANOS Y CICLO DE VIDA DEL COLABORADOR"
    ],
    "9": [
      "PSICOLOGÍA INSTRUCCIONAL",
      "DIAGNÓSTICO E INFORME PSICOLÓGICO",
      "INTRODUCCIÓN A LA PSICOTERAPIA II",
      "ESTRATEGIAS Y PROGRAMAS DE INTERVENCIÓN EN PROBLEMAS PSICOSOCIALES",
      "CAPACITACIÓN Y DESARROLLO DEL TALENTO"
    ],
    "10": [
      "ORIENTACIÓN VOCACIONAL Y OCUPACIONAL",
      "PSICOLOGÍA DE LA EXCEPCIONALIDAD",
      "ÉTICA PROFESIONAL",
      "DISEÑO Y EVALUACIÓN DE PROYECTOS",
      "CONSEJERÍA PSICOLÓGICA",
      "DIAGNÓSTICO DE CULTURA Y CLIMA ORGANIZACIONAL"
    ],
    "11": [
      "SEMINARIO DE INVESTIGACIÓN I",
      "INTERNADO I",
      "TEMAS ACTUALES EN PSICOLOGÍA I"
    ],
    "12": [
      "SEMINARIO DE INVESTIGACIÓN II",
      "INTERNADO II",
      "TEMAS ACTUALES EN PSICOLOGÍA II"
    ]
  }
};

export const prerequisitosGenerados = {
  "Administración": {
    "Estadística para la Gestión Empresarial I": [
      "Estadística Básica para los Negocios"
    ],
    "Planeamiento Estratégico I": [
      "Fundamentos de Administración"
    ],
    "Costos y Presupuestos": [
      "Contabilidad General"
    ],
    "Estadística para la Gestión Empresarial II": [
      "Estadística para la Gestión Empresarial I"
    ],
    "Herramientas Informáticas Para La Gestión II": [
      "Herramientas Informáticas para la Gestión I"
    ],
    "Diseño Organizacional": [
      "Planeamiento Estratégico I"
    ],
    "Gestión de Personas": [
      "Fundamentos de Administración"
    ],
    "Elaboración de Estados Financieros": [
      "Costos y Presupuestos"
    ],
    "Gestión Legal Empresarial": [
      "Planeamiento Estratégico I"
    ],
    "Marketing Estratégico": [
      "Introducción al Marketing"
    ],
    "Macroeconomía": [
      "Microeconomía"
    ],
    "Planeamiento Estratégico II": [
      "Diseño Organizacional"
    ],
    "Análisis de la Información Financiera": [
      "Elaboración de Estados Financieros"
    ],
    "Comportamiento Organizacional": [
      "Gestión de Personas"
    ],
    "Investigación y Análisis de Mercado": [
      "Marketing Estratégico"
    ],
    "Administración de Operaciones": [
      "Diseño Organizacional"
    ],
    "Tecnologías de Información para la Gestión": [
      "Herramientas Informáticas Para La Gestión II"
    ],
    "Taller de Innovación y Creatividad Empresarial": [
      "Planeamiento Estratégico II"
    ],
    "Logística y Comercio Internacional": [
      "Administración de Operaciones"
    ],
    "Gestión Financiera": [
      "Análisis de la Información Financiera"
    ],
    "Taller de Liderazgo Y Habilidades Gerenciales": [
      "Comportamiento Organizacional"
    ],
    "Legislación Laboral": [
      "Gestión de Personas"
    ],
    "Gestión Comercial": [
      "Investigación y Análisis de Mercado"
    ],
    "Taller de Emprendimiento y Creación De Negocios": [
      "Gestión Financiera"
    ],
    "Gestión De La Cadena De Suministro": [
      "Logística y Comercio Internacional"
    ],
    "Mercado De Valores": [
      "Gestión Financiera"
    ],
    "Ética Y Sostenibilidad Empresarial": [
      "Legislación Laboral"
    ],
    "Taller De Investigación Empresarial": [
      "Planeamiento Estratégico II"
    ],
    "Inteligencia De Negocios": [
      "Tecnologías de Información para la Gestión"
    ],
    "Gerencia De Operaciones": [
      "Gestión De La Cadena De Suministro"
    ],
    "Gerencia Financiera": [
      "Mercado De Valores"
    ],
    "Seminario De Investigación En Administración I": [
      "Taller De Investigación Empresarial"
    ],
    "Dirección Estratégica": [
      "Gerencia Financiera"
    ],
    "Gerencia De Personas": [
      "Taller de Liderazgo Y Habilidades Gerenciales"
    ],
    "Seminario De Investigación En Administración II": [
      "Seminario De Investigación En Administración I"
    ],
    "Gerencia De Marketing": [
      "Gestión Comercial"
    ]
  },
  "Arquitectura": {
    "Dibujo III": [
      "Dibujo II"
    ],
    "Física": [
      "Matemática para Arquitectura"
    ],
    "Urbanismo y Medio Ambiente I": [
      "Introducción al Comercio Internacional"
    ],
    "Proyecto de Arquitectura III": [
      "Proyecto de Arquitectura II"
    ],
    "Edificación Sostenible II": [
      "Edificación Sostenible I"
    ],
    "Grafica Digital": [
      "Dibujo III"
    ],
    "Historia y Teoría de la Arquitectura I": [
      "Introducción a la Arquitectura"
    ],
    "Orientación Estructural": [
      "Física"
    ],
    "Tecnología Ambiental I": [
      "Edificación Sostenible I"
    ],
    "Proyecto de Arquitectura IV": [
      "Proyecto de Arquitectura III"
    ],
    "Dibujo y presentación de Proyectos": [
      "Dibujo III"
    ],
    "Diseño Estructural I": [
      "Orientación Estructural"
    ],
    "Historia y Teoría de la Arquitectura II": [
      "Historia y Teoría de la Arquitectura I"
    ],
    "Instalaciones I": [
      "Edificación Sostenible II"
    ],
    "Urbanismo y Medio Ambiente II": [
      "Urbanismo y Medio Ambiente I"
    ],
    "Proyecto de Arquitectura V": [
      "Proyecto de Arquitectura IV"
    ],
    "Diseño Estructural II": [
      "Diseño Estructural I"
    ],
    "Historia y Teoría de la Arquitectura III": [
      "Historia y Teoría de la Arquitectura II"
    ],
    "Instalaciones II": [
      "Instalaciones I"
    ],
    "Proyecto de Arquitectura VI": [
      "Proyecto de Arquitectura V"
    ],
    "Gestión de Proyectos I": [
      "Estadística"
    ],
    "Historia de la Arquitectura Peruana": [
      "Historia y Teoría de la Arquitectura III"
    ],
    "Metrados, Presupuesto y Programación de Obra": [
      "Instalaciones II"
    ],
    "Proyecto de Arquitectura VII": [
      "Proyecto de Arquitectura VI"
    ],
    "Desarrollo de Proyecto": [
      "Instalaciones II"
    ],
    "Gestión de Proyectos II": [
      "Gestión de Proyectos I"
    ],
    "Proyecto de Arquitectura VIII": [
      "Proyecto de Arquitectura VII"
    ],
    "Seminario de Investigación en Arquitectura": [
      "Historia de la Arquitectura Peruana"
    ]
  },
  "Comunicaciones": {
    "Storytelling": [
      "Lenguaje y Comunicación II"
    ],
    "Bases de la Información": [
      "Storytelling"
    ],
    "Comunicación y Desarrollo": [
      "Fundamentos de la Comunicación"
    ],
    "Técnicas Fotográficas": [
      "Lenguaje Gráfico"
    ],
    "Comunicación y Empresa": [
      "Fundamentos de la Comunicación"
    ],
    "Sociedad y Comunicación": [
      "Fundamentos de la Comunicación"
    ],
    "Narrativa Audiovisual": [
      "Storytelling"
    ],
    "Técnicas de Audio": [
      "Técnicas Audiovisuales"
    ],
    "Técnicas de Diseño": [
      "Lenguaje Gráfico"
    ],
    "Técnicas de Investigación": [
      "Sociedad y Comunicación"
    ],
    "Taller de Video": [
      "Técnicas Audiovisuales",
      "Narrativa Audiovisual"
    ],
    "Comunicación y Experiencia de Usuario": [
      "Técnicas de Diseño"
    ],
    "Procesos Interculturales": [
      "Sociedad y Comunicación"
    ],
    "Comunicación Política": [
      "Sociedad y Comunicación"
    ],
    "Educación y Comunicación": [
      "Sociedad y Comunicación"
    ],
    "Semiótica Narrativa": [
      "Narrativa Audiovisual"
    ],
    "Gestión Empresarial": [
      "Técnicas de Investigación"
    ],
    "Legislación y Ética Profesional": [
      "Comunicación Política"
    ]
  },
  "Contabilidad y Finanzas": {
    "Fundamentos de Administración": [
      "Economía y Empresa"
    ],
    "Estadística para la Gestión Empresarial I": [
      "Estadística Básica para los Negocios"
    ],
    "Fundamentos de Finanzas": [
      "Matemática Aplicada a los Negocios II"
    ],
    "Contabilidad General": [
      "Matemática Aplicada a los Negocios II"
    ],
    "Herramientas Informáticas para la Gestión I": [
      "Economía y Empresa"
    ],
    "Estadística para la Gestión Empresarial II": [
      "Estadística para la Gestión Empresarial I"
    ],
    "Planeamiento Estratégico": [
      "Fundamentos de Administración"
    ],
    "Microeconomía": [
      "Economía y Empresa"
    ],
    "Costos y Presupuestos": [
      "Contabilidad General"
    ],
    "Herramientas Informáticas para la Gestión II": [
      "Herramientas Informáticas para la Gestión I"
    ],
    "Principios de Contabilidad Financiera": [
      "Contabilidad General"
    ],
    "Macroeconomía": [
      "Microeconomía"
    ],
    "Elaboración de Estados Financieros": [
      "Costos y Presupuestos"
    ],
    "Contabilidad Gerencial": [
      "Costos y Presupuestos"
    ],
    "Gestión Legal Empresarial": [
      "Planeamiento Estratégico"
    ],
    "Diseño Organizacional": [
      "Planeamiento Estratégico"
    ],
    "Gestión Tributaria": [
      "Principios de Contabilidad Financiera"
    ],
    "Análisis y Planeamiento Financiero": [
      "Elaboración de Estados Financieros"
    ],
    "Auditoría Interna y Cumplimiento": [
      "Diseño Organizacional"
    ],
    "Contabilidad Financiera Intermedia": [
      "Principios de Contabilidad Financiera"
    ],
    "Impuesto a la Renta": [
      "Gestión Tributaria"
    ],
    "Tecnologías de la Información para la Gestión": [
      "Herramientas Informáticas para la Gestión II"
    ],
    "Auditoría Financiera I": [
      "Auditoría Interna y Cumplimiento"
    ],
    "Finanzas Corporativas I": [
      "Análisis y Planeamiento Financiero"
    ],
    "Legislación Laboral": [
      "Gestión Legal Empresarial"
    ],
    "Impuestos Indirectos": [
      "Impuesto a la Renta"
    ],
    "Taller de Liderazgo y Habilidades Gerenciales": [
      "Diseño Organizacional"
    ],
    "Contabilidad Internacional para el Sector Público": [
      "Contabilidad Financiera Intermedia"
    ],
    "Gestión Bancaria": [
      "Análisis y Planeamiento Financiero"
    ],
    "Seminario de Informática Contable": [
      "Herramientas Informáticas para la Gestión II"
    ],
    "Banca de Inversión": [
      "Finanzas Corporativas I"
    ],
    "Introducción a la Investigación Contable": [
      "Contabilidad Financiera Intermedia"
    ],
    "Contabilidad Financiera Avanzada": [
      "Contabilidad Financiera Intermedia"
    ],
    "Taller de Emprendimiento y Creación de Negocios": [
      "Taller de Liderazgo y Habilidades Gerenciales"
    ],
    "Ética y Sostenibilidad Empresarial": [
      "Legislación Laboral"
    ],
    "Gerencia de Activos Financieros": [
      "Finanzas Corporativas I"
    ],
    "Tributación Sectorial": [
      "Impuesto a la Renta"
    ],
    "ERP para la Gestión Empresarial": [
      "Tecnologías de la Información para la Gestión"
    ],
    "Auditoría Forense": [
      "Auditoría Financiera I"
    ]
  },
  "Derecho": {
    "Expresión Escrita": [
      "Lenguaje y Comunicación II"
    ],
    "Problemática Nacional": [
      "Economía y Empresa"
    ],
    "Derecho Civil II (Personas Jurídicas)": [
      "Derecho Civil I (Principios Generales y Personas Naturales)"
    ],
    "Introducción al Derecho Procesal": [
      "Introducción a las Ciencias Jurídicas"
    ],
    "Derecho Romano": [
      "Historia General del Derecho"
    ],
    "Derecho Penal I (Parte General)": [
      "Introducción a las Ciencias Jurídicas"
    ],
    "Derecho Constitucional I (Derecho Constitucional General)": [
      "Introducción a la Ciencia Política"
    ],
    "Lógica y Argumentación Jurídica": [
      "Introducción a las Ciencias Jurídicas"
    ],
    "Derecho Penal II": [
      "Derecho Penal I (Parte General)"
    ],
    "Derecho Civil III (Acto Jurídico)": [
      "Derecho Civil II (Personas Jurídicas)"
    ],
    "Derecho Constitucional II (Derecho Constitucional Peruano)": [
      "Derecho Constitucional I (Derecho Constitucional General)"
    ],
    "Filosofía del Derecho": [
      "Filosofía Aplicada",
      "Introducción a las Ciencias Jurídicas"
    ],
    "Derecho Procesal Civil I": [
      "Introducción al Derecho Procesal"
    ],
    "Introducción a la Economía y Derecho": [
      "Economía y Empresa",
      "Introducción a las Ciencias Jurídicas"
    ],
    "Derecho Administrativo I": [
      "Derecho Constitucional II (Derecho Constitucional Peruano)"
    ],
    "Derecho Procesal Penal I": [
      "Introducción al Derecho Procesal",
      "Derecho Penal II"
    ],
    "Derecho Procesal Constitucional": [
      "Derecho Constitucional II (Derecho Constitucional Peruano)",
      "Derecho Procesal Civil I"
    ],
    "Derecho Civil IV (Reales)": [
      "Derecho Civil III (Acto Jurídico)"
    ],
    "Derecho Procesal Civil II": [
      "Derecho Procesal Civil I"
    ],
    "Derecho Civil V (Familia)": [
      "Derecho Civil IV (Reales)"
    ],
    "Derecho Administrativo II": [
      "Derecho Administrativo I"
    ],
    "Derecho Comercial I (Parte General)": [
      "Derecho Civil IV (Reales)"
    ],
    "Derecho Procesal Penal II": [
      "Derecho Procesal Penal I"
    ],
    "Derecho Civil VI (Obligaciones)": [
      "Derecho Civil IV (Reales)"
    ],
    "Derecho Procesal Civil III": [
      "Derecho Procesal Civil II"
    ],
    "Derecho Civil VIII (Sucesiones)": [
      "Derecho Civil VI (Obligaciones)",
      "Derecho Civil V (Familia)"
    ],
    "Derecho Laboral I (Parte General)": [
      "Derecho Administrativo II",
      "Derecho Civil VI (Obligaciones)"
    ],
    "Derecho Comercial II (Sociedades I)": [
      "Derecho Comercial I (Parte General)"
    ],
    "Derecho Civil VII (Contratos Parte General)": [
      "Derecho Civil VI (Obligaciones)"
    ],
    "Derecho Tributario I (Código Tributario)": [
      "Derecho Administrativo II"
    ],
    "Derecho Tributario II": [
      "Derecho Tributario I (Código Tributario)"
    ],
    "Derecho Civil IX (Contratos Típicos)": [
      "Derecho Civil VII (Contratos Parte General)"
    ],
    "Derecho Penal III (Penal Económico)": [
      "Derecho Penal II",
      "Derecho Tributario I (Código Tributario)",
      "Derecho Comercial II (Sociedades I)"
    ],
    "Derecho Comercial III (Sociedades II)": [
      "Derecho Comercial II (Sociedades I)"
    ],
    "Derecho Laboral II (Derecho Laboral Individual)": [
      "Derecho Laboral I (Parte General)"
    ],
    "Derecho Civil X (Garantías)": [
      "Derecho Civil VII (Contratos Parte General)"
    ],
    "Seminario de Investigación I": [
      "Derecho Tributario I (Código Tributario)",
      "Derecho Comercial II (Sociedades I)",
      "Derecho Civil IX (Contratos Típicos)"
    ],
    "Derecho Internacional Público": [
      "Derecho Constitucional II (Derecho Constitucional Peruano)"
    ],
    "Arbitraje": [
      "Derecho Comercial II (Sociedades I)",
      "Derecho Procesal Civil III"
    ],
    "Derecho Comercial IV (Títulos Valores)": [
      "Derecho Comercial II (Sociedades I)",
      "Derecho Civil VII (Contratos Parte General)"
    ],
    "Ética Profesional": [
      "Filosofía del Derecho"
    ],
    "Derecho Registral y Notarial": [
      "Derecho Comercial II (Sociedades I)",
      "Derecho Civil IX (Contratos Típicos)"
    ],
    "Derecho Civil XI (Responsabilidad Civil)": [
      "Derecho Civil IX (Contratos Típicos)"
    ],
    "Introducción a las Técnicas de Litigación Oral": [
      "Derecho Procesal Civil III"
    ],
    "Seminario de Investigación II": [
      "Seminario de Investigación I"
    ],
    "Derecho Internacional Privado": [
      "Derecho Civil X (Garantías)"
    ]
  },
  "Economía": {
    "Contabilidad para Economistas": [
      "Cálculo I"
    ],
    "Métodos Matemáticos I": [
      "Cálculo I"
    ],
    "Estadística General": [
      "Álgebra Lineal"
    ],
    "Historia del Pensamiento Económico": [
      "Fundamentos de Economía"
    ],
    "Herramientas Informáticas para Economistas": [
      "Álgebra Lineal"
    ],
    "Introducción a la Microeconomía": [
      "Cálculo I"
    ],
    "Teoría Microeconómica I": [
      "Introducción a la Microeconomía"
    ],
    "Estadística Aplicada I": [
      "Estadística General"
    ],
    "Teoría Macroeconómica I": [
      "Introducción a la Microeconomía"
    ],
    "Métodos Matemáticos II": [
      "Métodos Matemáticos I"
    ],
    "Ciencia de Datos": [
      "Herramientas Informáticas para Economistas"
    ],
    "Historia de la Economía Peruana": [
      "Historia del Pensamiento Económico"
    ],
    "Teoría Microeconómica II": [
      "Teoría Microeconómica I"
    ],
    "Estadística Aplicada II": [
      "Estadística Aplicada I"
    ],
    "Teoría Macroeconómica II": [
      "Teoría Macroeconómica I"
    ],
    "Métodos Matemáticos III": [
      "Métodos Matemáticos II"
    ],
    "Matemáticas Financieras y Actuariales": [
      "Métodos Matemáticos II"
    ],
    "Análisis Financiero": [
      "Matemáticas Financieras y Actuariales"
    ],
    "Economía Internacional": [
      "Teoría Microeconómica II"
    ],
    "Teoría Macroeconómica III": [
      "Teoría Macroeconómica II"
    ],
    "Métodos Matemáticos IV": [
      "Métodos Matemáticos III"
    ],
    "Teoría de Juegos": [
      "Teoría Microeconómica II"
    ],
    "Econometría I": [
      "Estadística Aplicada II"
    ],
    "Economía Financiera I": [
      "Análisis Financiero"
    ],
    "Organización Industrial": [
      "Economía Internacional"
    ],
    "Teoría y Política Monetaria": [
      "Teoría Macroeconómica III"
    ],
    "Economía Peruana Sectorial": [
      "Teoría Macroeconómica III"
    ],
    "Teoría Bancaria": [
      "Teoría de Juegos"
    ],
    "Econometría II": [
      "Econometría I"
    ],
    "Evaluación Económica de Políticas": [
      "Econometría II"
    ],
    "Economía de la Elección Pública": [
      "Teoría y Política Monetaria"
    ],
    "Crecimiento y Desarrollo Económico": [
      "Teoría y Política Monetaria"
    ],
    "Mercado de Capitales": [
      "Economía Financiera I"
    ],
    "Tópicos Avanzados en Economía y Finanzas": [
      "Econometría II"
    ],
    "Economía Financiera II": [
      "Economía Financiera I"
    ],
    "Seminario de Investigación I": [
      "Tópicos Avanzados en Economía y Finanzas"
    ],
    "Política Económica": [
      "Economía de la Elección Pública"
    ],
    "Economía Financiera Aplicada": [
      "Economía Financiera II"
    ],
    "Ética para Economistas": [
      "Economía de la Elección Pública"
    ],
    "Evaluación Privada de Proyectos": [
      "Economía Financiera II"
    ],
    "Seminario de Investigación II": [
      "Seminario de Investigación I"
    ],
    "Tópicos en Investigación Económica": [
      "Política Económica"
    ]
  },
  "Ingeniería Ambiental": {
    "Física I": [
      "Cálculo I"
    ],
    "Cálculo II": [
      "Cálculo I"
    ],
    "Principios de Ingeniería Sostenible": [
      "Fundamentos de Programación"
    ],
    "Estadística y Probabilidad": [
      "Cálculo II"
    ],
    "Física II": [
      "Física I"
    ],
    "Química Ambiental": [
      "Química General"
    ],
    "Cálculo III": [
      "Cálculo II"
    ],
    "Estadística Aplicada": [
      "Estadística y Probabilidad"
    ],
    "Derecho y Legislación Ambiental": [
      "Principios de Ingeniería Sostenible"
    ],
    "Ciencia del Suelo": [
      "Química Ambiental"
    ],
    "Técnicas Instrumentales de Análisis": [
      "Química Ambiental"
    ],
    "Gestión Sostenible de Residuos": [
      "Derecho y Legislación Ambiental"
    ],
    "Técnicas de Control de Contaminación del Aire": [
      "Estadística Aplicada"
    ],
    "Energías Renovables": [
      "Física II"
    ],
    "Mecánica de Fluidos": [
      "Física II"
    ],
    "Microbiología Ambiental": [
      "Biología Ambiental"
    ],
    "Técnicas de Control de la Contaminación del Agua": [
      "Mecánica de Fluidos"
    ],
    "Sistemas Integrados de Gestión": [
      "Derecho y Legislación Ambiental"
    ],
    "Economía Ambiental": [
      "Costeo de Proyectos Ambientales"
    ],
    "Evaluación del Impacto Ambiental": [
      "Recursos Naturales y Ecosistemas"
    ],
    "Técnicas de Control de la Contaminación del Suelo": [
      "Recursos Naturales y Ecosistemas"
    ],
    "Proyecto de Ingeniería Aplicada I": [
      "Inteligencia de Negocios"
    ],
    "Formulación y Diseño de Proyectos Sostenibles": [
      "Evaluación del Impacto Ambiental"
    ],
    "Proyecto de Ingeniería Aplicada II": [
      "Proyecto de Ingeniería Aplicada I"
    ]
  },
  "Ingeniería Civil": {
    "Cálculo II": [
      "Cálculo I"
    ],
    "Probabilidad y Estadística Aplicada": [
      "Cálculo I"
    ],
    "Física I": [
      "Álgebra Lineal"
    ],
    "Geología Aplicada": [
      "Topografía"
    ],
    "Modelación de Información de Edificaciones I": [
      "Ingeniería Gráfica"
    ],
    "Física II": [
      "Física I"
    ],
    "Ecuaciones Diferenciales": [
      "Cálculo II"
    ],
    "Tecnología del Concreto": [
      "Química Aplicada"
    ],
    "Programación Digital": [
      "Probabilidad y Estadística Aplicada"
    ],
    "Mecánica de Suelos I": [
      "Geología Aplicada"
    ],
    "Dinámica": [
      "Ecuaciones Diferenciales"
    ],
    "Modelación de Información de Edificaciones II": [
      "Modelación de Información de Edificaciones I"
    ],
    "Estática": [
      "Física I"
    ],
    "Tecnología de Construcción II": [
      "Tecnología de Construcción I"
    ],
    "Ingeniería Ambiental": [
      "Tecnología de Construcción II"
    ],
    "Mecánica de Suelos II": [
      "Mecánica de Suelos I"
    ],
    "Tecnología de los Materiales": [
      "Tecnología del Concreto"
    ],
    "Instalaciones en Edificaciones": [
      "Modelación de Información de Edificaciones II"
    ],
    "Resistencia de Materiales": [
      "Estática"
    ],
    "Costos y Presupuestos": [
      "Tecnología de Construcción I"
    ],
    "Mecánica de Fluidos": [
      "Dinámica"
    ],
    "Gestión de Proyectos de Construcción": [
      "Costos y Presupuestos"
    ],
    "Ingeniería de Transporte I": [
      "Mecánica de Suelos II"
    ],
    "Fundamentos del Planeamiento Urbano y Regional": [
      "Tecnología de Construcción II"
    ],
    "Análisis Estructural I": [
      "Resistencia de Materiales"
    ],
    "Métodos Numéricos": [
      "Programación Digital"
    ],
    "Hidráulica": [
      "Mecánica de Fluidos"
    ],
    "Gestión de Proyectos de Operación y Mantenimiento": [
      "Gestión de Proyectos de Construcción"
    ],
    "Gestión Estratégica de Contratos": [
      "Gestión de Proyectos de Construcción"
    ],
    "Concreto Armado I": [
      "Resistencia de Materiales"
    ],
    "Ingeniería Sismorresistente": [
      "Análisis Estructural I"
    ],
    "Hidrología": [
      "Hidráulica"
    ],
    "Seminario de Investigación II": [
      "Seminario De Investigación I"
    ],
    "Estructuras Metálicas": [
      "Análisis Estructural I"
    ]
  },
  "Ingeniería Industrial": {
    "Diseño Asistido por el Computador": [
      "Álgebra Lineal"
    ],
    "Física I": [
      "Cálculo I"
    ],
    "Cálculo II": [
      "Cálculo I"
    ],
    "Costeo de Operaciones": [
      "Sistemas Organizacionales"
    ],
    "Fundamentos de Programación": [
      "Inteligencia Artificial Aplicada"
    ],
    "Física II": [
      "Física I"
    ],
    "Estadística y Probabilidad": [
      "Cálculo I"
    ],
    "Mecánica": [
      "Física I",
      "Diseño Asistido por el Computador"
    ],
    "Cálculo III": [
      "Cálculo II"
    ],
    "Investigación de Operaciones I": [
      "Fundamentos de Programación"
    ],
    "Fundamentos de Operaciones y Logística": [
      "Costeo de Operaciones"
    ],
    "Ingeniería Económica": [
      "Costeo de Operaciones"
    ],
    "Electricidad y Electrónica": [
      "Física II"
    ],
    "Ecuaciones Diferenciales": [
      "Cálculo III"
    ],
    "Diseño de Experimentos": [
      "Estadística y Probabilidad"
    ],
    "Investigación de Operaciones II": [
      "Investigación de Operaciones I"
    ],
    "Ergonomía y Diseño del Trabajo": [
      "Estadística y Probabilidad"
    ],
    "Innovación en Ingeniería": [
      "Electricidad y Electrónica",
      "Mecánica"
    ],
    "Planeamiento y Control de Operaciones": [
      "Fundamentos de Operaciones y Logística"
    ],
    "Procesos Industriales": [
      "Química General",
      "Física II"
    ],
    "Ingeniería Financiera": [
      "Ingeniería Económica"
    ],
    "Diseño de Instalaciones": [
      "Ergonomía y Diseño del Trabajo"
    ],
    "Modelos de Sistemas Logísticos": [
      "Fundamentos de Operaciones y Logística"
    ],
    "Calidad": [
      "Diseño de Experimentos"
    ],
    "Procesos de Manufactura": [
      "Innovación en Ingeniería"
    ],
    "Simulación de Procesos": [
      "Investigación de Operaciones II"
    ],
    "Modelamiento Predictivo de Datos": [
      "Inteligencia de Negocios"
    ],
    "Sistemas Integrados de Gestión": [
      "Calidad"
    ],
    "Análisis de Problemas de Ingeniería": [
      "Modelos de Sistemas Logísticos",
      "Calidad"
    ],
    "Automatización Industrial": [
      "Procesos de Manufactura"
    ],
    "Proyecto de Ingeniería Aplicada I": [
      "Análisis de Problemas de Ingeniería"
    ],
    "Ingeniería Comercial": [
      "Modelos de Sistemas Logísticos"
    ],
    "Proyecto de Ingeniería Aplicada II": [
      "Proyecto de Ingeniería Aplicada I",
      "Simulación de Procesos"
    ],
    "Gerencia Estratégica": [
      "Ingeniería Financiera",
      "Ingeniería Comercial"
    ]
  },
  "Ingeniería Mecatrónica": {
    "Cálculo II": [
      "Cálculo I"
    ],
    "Circuitos Digitales": [
      "Álgebra Lineal"
    ],
    "Dibujo Mecánico": [
      "Álgebra Lineal"
    ],
    "Ecuaciones Diferenciales": [
      "Cálculo II"
    ],
    "Estadística y Probabilidad": [
      "Cálculo II"
    ],
    "Circuitos Eléctricos": [
      "Circuitos Digitales"
    ],
    "Fundamentos de Máquinas y Mecanismos": [
      "Dibujo Mecánico"
    ],
    "Análisis Complejo y Transformadas": [
      "Ecuaciones Diferenciales"
    ],
    "Costos y Presupuestos": [
      "Sistemas Organizacionales"
    ],
    "Programación de Computadoras": [
      "Inteligencia Artificial Aplicada"
    ],
    "Circuitos Electrónicos": [
      "Circuitos Eléctricos"
    ],
    "Análisis de Señales y Sistemas": [
      "Análisis Complejo y Transformadas"
    ],
    "Tecnología de Materiales e Innovación": [
      "Ingeniería de Resistencia de Materiales"
    ],
    "Control de Sistemas Dinámicos I": [
      "Análisis Complejo y Transformadas"
    ],
    "Ingeniería de Fluidos y Calor": [
      "Mecánica Aplicada"
    ],
    "Microcontroladores": [
      "Programación de Computadoras"
    ],
    "Sensores y Actuadores Inteligentes": [
      "Circuitos Eléctricos"
    ],
    "Control de Sistemas Dinámicos II": [
      "Control de Sistemas Dinámicos I"
    ],
    "Proyecto Integrador de Mecatrónica I": [
      "Proyecto de Investigación"
    ],
    "Proyecto Integrador de Mecatrónica II": [
      "Proyecto Integrador de Mecatrónica I"
    ]
  },
  "Marketing": {
    "Estadística Empresarial I": [
      "Estadística Básica para los Negocios"
    ],
    "Herramientas Informáticas para la Gestión I": [
      "Economía y Empresa"
    ],
    "Costos y Presupuestos": [
      "Contabilidad General"
    ],
    "Sociología del Consumidor": [
      "Antropología del Consumidor"
    ],
    "Procesos de Marketing": [
      "Antropología del Consumidor"
    ],
    "Estadística Empresarial II": [
      "Estadística Empresarial I"
    ],
    "Administración Empresarial y Marketing": [
      "Economía y Empresa"
    ],
    "Microeconomía": [
      "Economía y Empresa"
    ],
    "Estadística Aplicada al Marketing": [
      "Estadística Empresarial II"
    ],
    "Consumidor y Experiencia del Cliente": [
      "Sociología del Consumidor"
    ],
    "Elaboración de Estados Financieros": [
      "Costos y Presupuestos",
      "Introducción a las Finanzas"
    ],
    "Administración de Canales de Distribución": [
      "Procesos de Marketing"
    ],
    "Gestión de Producto y Marca": [
      "Procesos de Marketing"
    ],
    "Macroeconomía": [
      "Microeconomía"
    ],
    "Política y Fijación de Precios": [
      "Macroeconomía"
    ],
    "Operaciones Logísticas de Marketing": [
      "Administración de Canales de Distribución"
    ],
    "Investigación Cuantitativa de Mercados": [
      "Estadística Aplicada al Marketing"
    ],
    "Publicidad y Medios": [
      "Gestión de Producto y Marca"
    ],
    "Branding": [
      "Gestión de Producto y Marca"
    ],
    "Investigación Cualitativa de Mercados": [
      "Consumidor y Experiencia del Cliente"
    ],
    "Ciencia de Datos Aplicado al Marketing": [
      "Estadística Aplicada al Marketing"
    ],
    "Dirección Estratégica de Marketing": [
      "Investigación Cuantitativa de Mercados"
    ],
    "Finanzas para Marketing": [
      "Elaboración de Estados Financieros"
    ],
    "Marketing Social": [
      "Investigación Cuantitativa de Mercados"
    ],
    "Comunicación e Imagen Corporativa": [
      "Publicidad y Medios"
    ],
    "Transformación Digital y Marketing": [
      "Publicidad y Medios"
    ],
    "Endomarketing y Gestión de Personas": [
      "Administración Empresarial y Marketing"
    ],
    "Gerencia de Ventas": [
      "Operaciones Logísticas de Marketing"
    ],
    "Ética y Sostenibilidad Empresarial": [
      "Endomarketing y Gestión de Personas"
    ],
    "Herramientas Estratégicas de Negocios y Marketing": [
      "Dirección Estratégica de Marketing"
    ],
    "Métodos y Técnicas de Evaluación de Proyectos": [
      "Finanzas para Marketing"
    ],
    "E-commerce": [
      "Transformación Digital y Marketing"
    ],
    "Trade Marketing y Merchandising": [
      "Gerencia de Ventas"
    ],
    "Marketing de Servicios": [
      "Marketing Social"
    ],
    "Seminario de Investigación en Marketing I": [
      "Herramientas Estratégicas de Negocios y Marketing"
    ],
    "Analítica de Datos": [
      "Transformación Digital y Marketing"
    ],
    "Negociación Comercial": [
      "Comunicación e Imagen Corporativa"
    ],
    "Seminario de Investigación en Marketing II": [
      "Seminario de Investigación en Marketing I"
    ],
    "Estrategias de Marketing Internacional": [
      "Trade Marketing y Merchandising"
    ],
    "Estrategia de Relacionamiento y Fidelización": [
      "Herramientas Estratégicas de Negocios y Marketing"
    ],
    "Plan de Marketing": [
      "Métodos y Técnicas de Evaluación de Proyectos"
    ],
    "Planeamiento Estratégico Publicitario": [
      "Publicidad y Medios"
    ]
  },
  "Negocios Internacionales": {
    "Estadística para la Gestión Empresarial I": [
      "Estadística Básica para los Negocios"
    ],
    "Cross-Cultural Management": [
      "Introducción al Comercio Internacional"
    ],
    "Introduction to International Business": [
      "Economía y Empresa"
    ],
    "Matemática para la Gestión de Negocios": [
      "Matemática Aplicada a los Negocios II"
    ],
    "International Relations": [
      "Introducción al Comercio Internacional"
    ],
    "Contabilidad General": [
      "Matemática Aplicada a los Negocios II"
    ],
    "Regulation of International Business Transactions": [
      "Introduction to International Business"
    ],
    "Microeconomía": [
      "Introduction to International Business"
    ],
    "Estadística para la Gestión Empresarial II": [
      "Estadística para la Gestión Empresarial I"
    ],
    "Economic Integration Processes": [
      "International Relations"
    ],
    "Marketing Internacional, Estudio de Mercado y Estrategias de Ventas": [
      "Estadística para la Gestión Empresarial I"
    ],
    "Costos y Presupuestos": [
      "Contabilidad General"
    ],
    "International Business Workshop I": [
      "Lenguaje y Comunicación II"
    ],
    "Exports, Imports and Customs": [
      "Regulation of International Business Transactions"
    ],
    "Macroeconomía": [
      "Microeconomía"
    ],
    "Métodos Cuantitativos para la Investigación en Negocios Internacionales": [
      "Estadística para la Gestión Empresarial II"
    ],
    "Inteligencia en los Negocios y Big Data": [
      "Marketing Internacional, Estudio de Mercado y Estrategias de Ventas"
    ],
    "Inteligencia de Mercados Internacionales": [
      "Marketing Internacional, Estudio de Mercado y Estrategias de Ventas"
    ],
    "Emprendimiento de Negocios Globales y Digitales": [
      "Marketing Internacional, Estudio de Mercado y Estrategias de Ventas"
    ],
    "International Business Workshop II": [
      "Introduction to International Business"
    ],
    "International Logistics I": [
      "Exports, Imports and Customs"
    ],
    "Teoría del Comercio Internacional": [
      "Macroeconomía"
    ],
    "International Business Strategies and Sustainability": [
      "Macroeconomía"
    ],
    "Desarrollo e Innovación de Servicios Internacionales": [
      "Macroeconomía"
    ],
    "Comportamiento de Mercados Internacionales": [
      "Inteligencia de Mercados Internacionales"
    ],
    "Finanzas Empresariales I": [
      "Matemática para la Gestión de Negocios"
    ],
    "Data Analytics Aplicado a los Negocios Internacionales": [
      "Inteligencia en los Negocios y Big Data"
    ],
    "Taller de Inteligencia de Mercados": [
      "Inteligencia de Mercados Internacionales"
    ],
    "Cooperación Internacional para el Desarrollo Sostenible": [
      "Economic Integration Processes"
    ],
    "International Logistics II": [
      "International Logistics I"
    ],
    "Estrategia de Producción y Precios Internacionales": [
      "International Business Strategies and Sustainability"
    ],
    "Global Standards Requirements": [
      "International Business Strategies and Sustainability"
    ],
    "Estrategias Aplicadas para la Negociación Internacional": [
      "Economic Integration Processes"
    ],
    "Innovation and International Product Development": [
      "Comportamiento de Mercados Internacionales"
    ],
    "Finanzas Empresariales II": [
      "Finanzas Empresariales I"
    ],
    "Taller de Liderazgo y Habilidades Gerenciales": [
      "Teoría del Comercio Internacional"
    ]
  },
  "Psicología": {
    "REALIDAD NACIONAL": [
      "Economía y Empresa"
    ],
    "REDACCIÓN CIENTÍFICA": [
      "Lenguaje y Comunicación II"
    ],
    "FILOSOFÍA DE LA CIENCIA": [
      "Filosofía Aplicada"
    ],
    "PROCESOS COGNITIVOS SUPERIORES": [
      "PROCESOS COGNITIVOS BÁSICOS"
    ],
    "DESARROLLO HUMANO I": [
      "PROCESOS COGNITIVOS BÁSICOS"
    ],
    "PSICOLOGÍA DE LA PERSONALIDAD": [
      "PSICOBIOLOGÍA HUMANA I"
    ],
    "PSICOBIOLOGÍA HUMANA II": [
      "PSICOBIOLOGÍA HUMANA I"
    ],
    "SEXUALIDAD HUMANA": [
      "PSICOBIOLOGÍA HUMANA I"
    ],
    "HISTORIA Y SISTEMAS PSICOLÓGICOS": [
      "FILOSOFÍA DE LA CIENCIA"
    ],
    "PSICOLOGÍA DEL APRENDIZAJE": [
      "PROCESOS COGNITIVOS SUPERIORES"
    ],
    "DESAROLLO HUMANO II": [
      "DESARROLLO HUMANO I"
    ],
    "TÉCNICAS DE ENTREVISTA Y OBSERVACIÓN": [
      "PSICOLOGÍA DE LA PERSONALIDAD"
    ],
    "PSICOLOGÍA DE LA SALUD": [
      "CONDUCTA SOCIAL"
    ],
    "TÉCNICAS DE GRUPOS": [
      "CONDUCTA SOCIAL"
    ],
    "EVALUACIÓN PSICOMÉTRICA DE PROCESOS COGNITIVOS": [
      "TÉCNICAS DE ENTREVISTA Y OBSERVACIÓN"
    ],
    "METODOLOGÍA DE LA INVESTIGACIÓN CUANTITATIVA": [
      "ESTADÍSTICA APLICADA A LA PSICOLOGÍA"
    ],
    "PSICOPATOLOGÍA": [
      "PSICOLOGÍA DE LA SALUD"
    ],
    "COMPORTAMIENTO ORGANIZACIONAL I": [
      "TÉCNICAS DE GRUPOS"
    ],
    "EPISTEMOLOGÍA DE LA PSICOLOGÍA": [
      "HISTORIA Y SISTEMAS PSICOLÓGICOS"
    ],
    "PSICOLOGÍA EDUCATIVA": [
      "PSICOLOGÍA DEL APRENDIZAJE"
    ],
    "EVALUACIÓN PSICOMÉTRICA DE LA PERSONALIDAD": [
      "EVALUACIÓN PSICOMÉTRICA DE PROCESOS COGNITIVOS"
    ],
    "METODOLOGÍA DE LA INVESTIGACIÓN CUALITATIVA": [
      "METODOLOGÍA DE LA INVESTIGACIÓN CUANTITATIVA"
    ],
    "EVALUACIÓN PROYECTIVA DE LA PERSONALIDAD": [
      "PSICOPATOLOGÍA"
    ],
    "COMPORTAMIENTO ORGANIZACIONAL II": [
      "COMPORTAMIENTO ORGANIZACIONAL I"
    ],
    "EVALUACIÓN PSICOPEDAGÓGICA": [
      "PSICOLOGÍA EDUCATIVA"
    ],
    "DISEÑO Y CONSTRUCCIÓN DE INSTRUMENTOS PSICOLÓGICOS": [
      "EVALUACIÓN PSICOMÉTRICA DE LA PERSONALIDAD"
    ],
    "INTRODUCCIÓN A LA PSICOTERAPIA I": [
      "EVALUACIÓN PROYECTIVA DE LA PERSONALIDAD"
    ],
    "PROCESOS DE RECURSOS HUMANOS Y CICLO DE VIDA DEL COLABORADOR": [
      "COMPORTAMIENTO ORGANIZACIONAL II"
    ],
    "PSICOLOGÍA INSTRUCCIONAL": [
      "EVALUACIÓN PSICOPEDAGÓGICA"
    ],
    "DIAGNÓSTICO E INFORME PSICOLÓGICO": [
      "EVALUACIÓN PROYECTIVA DE LA PERSONALIDAD"
    ],
    "INTRODUCCIÓN A LA PSICOTERAPIA II": [
      "INTRODUCCIÓN A LA PSICOTERAPIA I"
    ],
    "ESTRATEGIAS Y PROGRAMAS DE INTERVENCIÓN EN PROBLEMAS PSICOSOCIALES": [
      "PSICOLOGÍA COMUNITARIA"
    ],
    "CAPACITACIÓN Y DESARROLLO DEL TALENTO": [
      "PROCESOS DE RECURSOS HUMANOS Y CICLO DE VIDA DEL COLABORADOR"
    ],
    "ORIENTACIÓN VOCACIONAL Y OCUPACIONAL": [
      "PSICOLOGÍA INSTRUCCIONAL"
    ],
    "PSICOLOGÍA DE LA EXCEPCIONALIDAD": [
      "DIAGNÓSTICO E INFORME PSICOLÓGICO"
    ],
    "ÉTICA PROFESIONAL": [
      "EPISTEMOLOGÍA DE LA PSICOLOGÍA"
    ],
    "DISEÑO Y EVALUACIÓN DE PROYECTOS": [
      "DISEÑO Y CONSTRUCCIÓN DE INSTRUMENTOS PSICOLÓGICOS"
    ],
    "CONSEJERÍA PSICOLÓGICA": [
      "DIAGNÓSTICO E INFORME PSICOLÓGICO"
    ],
    "DIAGNÓSTICO DE CULTURA Y CLIMA ORGANIZACIONAL": [
      "CAPACITACIÓN Y DESARROLLO DEL TALENTO"
    ],
    "SEMINARIO DE INVESTIGACIÓN I": [
      "DISEÑO Y EVALUACIÓN DE PROYECTOS"
    ],
    "SEMINARIO DE INVESTIGACIÓN II": [
      "SEMINARIO DE INVESTIGACIÓN I"
    ],
    "INTERNADO II": [
      "INTERNADO I"
    ],
    "TEMAS ACTUALES EN PSICOLOGÍA II": [
      "TEMAS ACTUALES EN PSICOLOGÍA I"
    ]
  }
};
